...
static const struct SubQuest sSubQuests1[QUEST_ALL_BADGES_SUB_COUNT] =
{
	sub_quest(
	      0,
...
void HandleQuestIconForSingleObjectEvent(struct ObjectEvent *objectEvent, u32 objectEventId)
{
	...
	// Do not have quest icons if quests have not been unlocked(only really needed for old lady in Mulberry)
	if (!FlagGet(FLAG_SYS_QUEST_MENU_GET))
		return;

	// Remove icon if quest is active
	if (QuestMenu_GetSetQuestState(questId, FLAG_GET_ACTIVE))
	{
